...
class FluidStaticSolver:
    ...
    def step(self, fluid: FluidState, statics: List[StaticBodyState], dt: float) -> None:
        ...
        for idx, position in enumerate(fluid.positions):
            ...
            for static in statics:
                ...
                if not self._inside(position, bounds_min, bounds_max):
                    continue
    ...
    def _inside(self, position: Vec3, bounds_min: Vec3, bounds_max: Vec3) -> bool:
        return (
            bounds_min[0] <= position[0] <= bounds_max[0]
            and bounds_min[1] <= position[1] <= bounds_max[1]
            and bounds_min[2] <= position[2] <= bounds_max[2]
        )
